...
def ReverseAESLastRound(ct_list_i,fct_list_i,cand_len,column,list_diff):
...
def MixColumnFaultPropagation(ct_list_i,fct_list_i,cand_len,column):
...
def round9_key_recovery(pt, ct, ct_list, fct_list, ln):
	...
	total_cand = dict()
	for i in range(0,ln):
		list_diff,column = MixColumnFaultPropagation(ct_list[i],fct_list[i],cand_len,column)
		candidates = ReverseAESLastRound(ct_list[i],fct_list[i],cand_len1,column,list_diff)
